...
class LocationViewController: UIViewController, MKMapViewDelegate, NSFetchedResultsControllerDelegate {
    ...
    var selectedAnnotation: LocationHelper!
    ...
    func handleGestureToAddLocation(gestureRecogniser: UIGestureRecognizer) {
        ...
        var newLocation = Location(dictionary: locationInformation, context: self.sharedContext)
    ...
    func addPinToMap(location: Location){
        let pinAnnotation = LocationHelper(annotationLocation: location)
        ...
        if location.photos.count == 0 {
            location.fetchForPhotos({ (success, errorString) -> Void in
                if success {
                    ...
                    if errorString != nil {
                        self.alertVC = DataHelper.raiseInformationalAlert(inViewController: self, withTitle: "Error", message: errorString!, completionHandler: { (alertAction) -> Void in
    ...
    func mapView(mapView: MKMapView, annotationView view: MKAnnotationView, didChangeDragState newState: MKAnnotationViewDragState, fromOldState oldState: MKAnnotationViewDragState) {
        if newState == MKAnnotationViewDragState.Ending {
            ...
            (view.annotation as! LocationHelper).location.fetchForPhotos({ (success, errorString) -> Void in
                if success {
                    let annot = view.annotation as! LocationHelper
    ...
    func mapView(mapView: MKMapView, viewForAnnotation annotation: MKAnnotation) -> MKAnnotationView! {
        ...
        if pinView == nil {
            ...
            if (annotation as! LocationHelper).location.photos.count == 0 {
    ...
    func showPhotosForLocation(sender: AnyObject) {
        ...
        if selectedAnnotation.location.isFetchingForPhotos == true {
            self.alertVC = DataHelper.raiseInformationalAlert(inViewController: self, withTitle: "Getting Ready...", message: "We are retrieving some cool photos of the location you just selected. It will be ready in a tick.", completionHandler: { (alertAction) -> Void in
                self.alertVC!.dismissViewControllerAnimated(true, completion: nil)
            })
        } else if (selectedAnnotation.location.photos.count == 0) {
            self.alertVC = DataHelper.raiseInformationalAlert(inViewController: self, withTitle: "No Images", message: "No images found for this location. Drag the pin to a new location.", completionHandler: { (alertAction) -> Void in
                self.alertVC!.dismissViewControllerAnimated(true, completion: nil)
            })
        } else {
    ...
    func mapView(mapView: MKMapView, didSelectAnnotationView view: MKAnnotationView) {
        selectedAnnotation = view.annotation as! LocationHelper
    }
    
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        (segue.destinationViewController as! PhotoViewController).displayLocation = selectedAnnotation.location
    }
