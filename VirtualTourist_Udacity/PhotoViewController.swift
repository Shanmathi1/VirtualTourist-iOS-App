...
class PhotoViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, NSFetchedResultsControllerDelegate {
    ...
    func controller(controller: NSFetchedResultsController, didChangeObject anObject: AnyObject, atIndexPath indexPath: NSIndexPath?, forChangeType type: NSFetchedResultsChangeType, newIndexPath: NSIndexPath?) {
        ...
        case .Update:
            let cell = self.photosCollectionView.cellForItemAtIndexPath(indexPath!) as! PhotoCell
    ...
    func configureCell(cell: PhotoCell,withPhoto photo: Photos) {
    ...
    @IBAction func retrieveNewCollectionOfPhotos(sender: AnyObject) {
        ...
        if displayLocation.totalNumberOfPhotos?.integerValue < MAX_NUMBER_OF_PHOTOS {
            self.alertVC = DataHelper.raiseInformationalAlert(inViewController: self, withTitle: "Error", message: "All available photos for this location have been retrieved.", completionHandler: { (alertAction) -> Void in
                self.alertVC!.dismissViewControllerAnimated(true, completion: nil)
            })
            return
        }
        ...
        for index in 1...photosCollectionView.numberOfItemsInSection(0) {
            let cell = photosCollectionView.cellForItemAtIndexPath(NSIndexPath(forRow: index - 1, inSection: 0))! as! PhotoCell
